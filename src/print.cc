...
{
    str = concat(make_identation(ident), L"[ make function name = `", target.func_name, L"`; return type = `",
                 repr(target.return_type), L"`; args = {\n");
...
void PrintVisitor::visit(const ForStatement &target)
{
    ...
    str +=
        concat(make_identation(ident + 1), L"start = {\n", start.result(), L"\n", make_identation(ident + 1), L"},\n");
...
{
    str = concat(make_identation(ident), L"[ get extern function name = `", target.func_name, L"`; return type = `",
                 repr(target.return_type), L"`; args = {\n");
    for (const auto &i : target.parameters) {
        str += concat(make_identation(ident + 2), L"name = `", i.name, L"`; type = `", repr(i.type), L"`,\n");
    }
