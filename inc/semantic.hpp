...
class SemanticAnalyser : public Visitor {
	...
	bool is_in_scope(const std::wstring &name,
	...
	[[noreturn]] void report_argument_number_mismatch(std::size_t expected, std::size_t got,
...
template <typename... Allowed> void SemanticAnalyser::report_bad_type(Allowed &&... allowed) const
{
	...
	throw SemanticException{ concat(position_in_file(position), L"\n In \n",
					...
					error_marker(position), L"\n\n", L"Error expected one of type `",
					repr(allowed...), L"` but instead got `", repr(got), L"`\n") };
}
