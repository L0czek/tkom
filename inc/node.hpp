...
struct FunctionCall : public Expression {
    ...
    public:
	FunctionCall(const Position &position, std::wstring func_name,
		     std::list<std::unique_ptr<Expression> > arguments)
...
struct ExternFunctionDecl : public Statement {
	...
	ExternFunctionDecl(const Position &pos, std::wstring name, BuiltinType return_type,
			   std::list<Parameter> parameters)
...
struct FunctionDecl : public Statement {
    ...
    public:
	FunctionDecl(const Position &position, std::wstring func_name, BuiltinType return_type,
		     std::list<Parameter> params, std::unique_ptr<Block> block)
		: pos(position), func_name(std::move(func_name)), return_type(return_type),
		  parameters(std::move(params)), block(std::move(block))
	{
	}
	void accept(Visitor &visitor) const override
	{
...
struct ForStatement : public Statement {
	...
	ForStatement(std::wstring loop_variable, const Position &loop_variable_pos, std::unique_ptr<Expression> start,
		     ...
		     std::unique_ptr<Block> block)
		: loop_variable(std::move(loop_variable)), loop_variable_pos(loop_variable_pos),
		  start(std::move(start)), end(std::move(end)), increase(std::move(increase)), block(std::move(block))
	{
	}
	void accept(Visitor &visitor) const override
	{
...
struct Program : public ASTNode {
    ...
    public:
	Program(std::list<std::unique_ptr<VariableDecl> > global_vars,
		std::list<std::unique_ptr<FunctionDecl> > functions,
		std::list<std::unique_ptr<ExternFunctionDecl> > externs)
